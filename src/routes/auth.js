...
const { DISALLOWED_USER_TYPES_BY_NAMES, DISALLOWED_ROLES_BY_NAMES } = require('../../config/constants');
...
router.post('/register', async (req, res) => {
  try {
    const { firstName, lastName, role, phoneNumber, email, password, userType } = req.body;
    ...
    try {
      const userTypeId = parseInt(userType); // Convert role to an integer

      userTypeFromConfig = getUserTypeById(userTypeId);
      console.log("userTypeFromConfig", userTypeFromConfig);
      if (!userTypeFromConfig) {
        console.error("userType does not exists", userTypeId, userTypeFromConfig)
    ...
    try {
      ...
      if (userTypeFromConfig.name !== validRole.name) {
        console.error("role and userType is not matching", userTypeFromConfig.name, validRole.name)
    ...
    // Create a new user
    const newUser = await appUser.create({ firstName, lastName, email, role, phoneNumber, userType });
